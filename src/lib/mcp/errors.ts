...
const RECOVERY_HINTS: Partial<Record<ServiceErrorCode, string>> = {
  ...
  COLUMN_NOT_FOUND: "No column with that id. Call board_get to list valid column ids.",
  APPLICATION_NOT_FOUND:
    "No application with that id. Call board_get to list valid application ids.",
  COLUMN_NOT_EMPTY: "Move or delete every application in this column first, then retry.",
  ...
  ENTRY_COLUMN_CATEGORY: "The entry column has a fixed category and cannot be changed.",
  ...
  AI_WATCH_CONFIG_NOT_FOUND: "No AI Watch config. Call ai_watch_save_config first.",
  OFFER_NOT_FOUND: "No offer with that id. Call ai_watch_list_offers for valid ids.",
  LLM_KEY_MISSING:
...
export async function withToolErrors<T>(
  ...
  try {
    ...
    return {
      content: [{ type: "text", text: "INTERNAL_ERROR: an unexpected error occurred." }],
      isError: true,
    };
  }
}
